...
// svcDB opens a fresh connection for test cleanup, since the runtime's own
// connection is closed by the deferred Shutdown before t.Cleanup runs.
func svcDB(t *testing.T) *sqlx.DB {
	t.Helper()
...
func TestDocuments_EndToEnd(t *testing.T) {
	...
	t.Cleanup(func() { _, _ = svcDB(t).Exec("DELETE FROM documents WHERE tenant_id = $1", tenant) })
